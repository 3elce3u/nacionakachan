...
class Novela{
	...
	function getpaginas($filtro,$typo){
		...
		else
		{
			if($typo=="busqueda")
			{
				$pre="SELECT novela_id from novelas where Match(novela_keywords) AGAINST ('$filtro')";
	...
	function getnovelabyid($obra_id){
        $con= new bbdd;
        $conexion=$con->conect();
        $consulta="SELECT * from novelas,tipos,estados,periodos,videos 
					where obras.obra_id=?
        ...
        $ok2=mysqli_stmt_execute($ok);
        $ok3=mysqli_stmt_bind_result($ok,$obra_id,$tipo_id,$tipo,$obra_nombre,$obra_generos,$obra_sinopsis,$obra_estado,$estado,$obra_periodo,$periodo,$obra_patrocinada,
	...
	function guardarvolumenes($novela_id,$volumen,$volumen_caratula,$volumen_miniatura,$volumen_fecha_original,$volumen_enlace_pdf,$volumen_enlace_video)
	{
		$con= new bbdd;
        $conexion=$con->conect();
        $consulta="insert into volumenes(volumen_id,novela_id,volumen,volumen_caratula,volumen_miniatura,volumen_fecha_publicacion,volumen_fecha_original,volumen_enlace_pdf,volumen_enlace_video)
				   values(NULL,?,?,?,?,NOW(),?,?,?)";
		$ok=mysqli_prepare($conexion,$consulta) or die ("Algo ha ido mal en la consulta a la base de datos novel paso 1");
		$params=mysqli_stmt_bind_param($ok,"issssss",$novela_id,$volumen,$volumen_caratula,$volumen_miniatura,$volumen_fecha_original,$volumen_enlace_pdf,$volumen_enlace_video);
        $exe=mysqli_stmt_execute($ok);
		@mysqli_stmt_close($ok);
	}
	...
	function editar_novela($novela_id,$escritor_nombre,$ilustrador_nombre,$editorial_nombre,$editorial_enlace,$fansub_nombre,
						   $fansub_enlace,$novela_nombre,$novela_nombre_original,$novela_generos,$novela_sinopsis,$novela_caratula,$novela_miniatura,$novela_keywords,$novela_fecha_publicacion)
	{
		$con= new bbdd;
        $conexion=$con->conect();
		$reg['novela_id']=$novela_id;
		...
		$reg['novela_fecha_publicacion']=$novela_fecha_publicacion;
					...
					for($a=$inicio;$a<$fin;$a++)
					{
						...
						if(!empty($pagina))
						{
							if($pagina==$a)
							{
								if($a>9)
								{
									$uno.= '<td class="boton3" style="background-color:rgba(160,237,98,0.30);">
					...
					for($a=$inicio;$a<$fin;$a++)
					{
						...
						if(!empty($pagina))
						{
							if($pagina==$a)
							{
								if($a>9)
								{
									$dos.= '<td class="boton3" style="background-color:rgba(160,237,98,0.30);">
									<div onclick="cazargenero(`'.$filtro.'`,`'.$typo.'`,'.$a.')"  class="paginet">
									'.$a.'
									</div>
									</td>';
