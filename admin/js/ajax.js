    ...
    function comentarios(novela_id,volumen_id){
        $.ajax({
            url:'ajax/comentarios.php',
            type:'POST',
            data:{novela_id,volumen_id},
                                ...
                                <div class="pre-coment" id="coment`+i+`">
                                    ...
                                    <div class="optcoment" id="optcoment"> 
                                        ...
                                        <div class="likecoment" id="likecoment">
                                           <img class='imglikes' src='recursos/iconos/like.png' height='25' width='25' onclick='guardarlike(`+comen.comentario_id+`,`+comen.novela_id+`,`+comen.volumen_id+`)'>  &nbsp;`+comen.likes+`
                                        </div>`;
                                        if(comen.margen<8)
                                        {
                                            temp=temp+`<div class="respcoment" id="respcoment" onclick='mostrarcajacomentarios(`+comen.comentario_id+`,`+comen.novela_id+`,`+comen.volumen_id+`)'>
                                                Responder
                                            </div>`; 
                                        }
                                        if(usuario_id==comen.usuario_id)
                                        {
                                            temp=temp+`<div class="respcoment" id="respcoment" onclick='editarcomentario(`+comen.comentario_id+`,`+comen.novela_id+`,`+comen.volumen_id+`);'>
                                                Editar
                                            </div>
                                            <div class="borrar">
                                                <img class='imgbasura' src='recursos/iconos/basura.png' height='25' width='25' onclick='borrarcomentario(`+comen.comentario_id+`,`+comen.novela_id+`,`+comen.volumen_id+`)'>
                                            </div>`;
                                        }
                            temp=temp+`</div>
                                </div>
    ...
    function editarcomentario(comentario_id,novela_id,volumen_id)
    {
        $.ajax({
            url:'ajax/cercomentario.php',
            type:'post',
            data:{comentario_id,novela_id,volumen_id},
        ...
        $('#novelaId2').val(novela_id);
        $('#volumenId2').val(volumen_id);
    }
    function borrarcomentario(comentario_id,novela_id,volumen_id)
    {
        $.ajax({
            type:'POST',
            ...
            success:function(response){
                $('#comentarios').empty();
                comentarios(novela_id,volumen_id)
            }
        });
    }
    function mostrarcajacomentarios(respuesta,novela_id,volumen_id)
    {
        $('#comentarfixed').css('display','block')
        $('#volumenId2').val(volumen_id);
        $('#novelaId2').val(novela_id);
    ...
    function guardarlike(comentario_id,novela_id,volumen_id){
        $.ajax({
            type:'POST',
            ...
            $.ajax({
                ...
                success:function(resp){
                    ...
                    respon.forEach(respo => {
                        template+=`<form method='GET' action='editarobra.php' id='no`+respo.obra_id+`'><input type='hidden' name='obra_id' value='`+respo.obra_id+`'>
    ...
    function novela(novela_id,novela_nombre)
    {
        $.ajax({
            type: "POST",
            url: "ajax/novela.php",
            data: {novela_id,novela_nombre},
            success: function (response) {
                let resp=JSON.parse(response);
                let volu=resp.volumenes.replace(/`/gi, "'");
                let template=`<div class='principalNovela'>
                    <div class='caratulaNovela'>`+resp.caratula+`</div>
                    <div class='nombreNovela'>`+resp.novela_nombre+`</div>
                    <div class='nombreOriginalNovela'>`+resp.nombre_original+`</div>
                    <div class='generosNovela'>`+resp.generos+`</div>
                    <div class='escritorNovela'>`+resp.nombre+`</div>
                    <div class='ilustradorNovela'>`+resp.ilustrador+`</div>
                    <div class='editorialNovela'>`+resp.editorial+`</div>
                    <div class='fansubNovela'>`+resp.fansub+`</div>
                ...
                <h2 style='margin-top:5px;'>VOLUMENES</h2><div style="width:100%; display:flex; flex-wrap:wrap; flex-direction:row; justify-content:center; ">`+volu+`</div>`;
    ...
    function volumen(novela_id,volumen_id)
    {
    var tiempo;
        $.ajax({
            type: "GET",
            ...
            success: function (response) {
                let resp=JSON.parse(response);
                template=`<div class="mainVolumen">
                                    <div class="pdf">
                                        <p class="boton" onclick="sumardescarga(`+resp.volumen_id+`,`+resp.novela_id+`);window.open('`+resp.volumen_enlace_pdf+`','_blank');">DESCARGAR PDF</p>
                                    </div>
                                    <div class="novnom">
                                        <b>`+resp.novela_nombre+`</b>
                                    </div>
                                    <div class="numvol" id="numvol"></div>
                                    <div class="carvol">
                                        <img height="320" width="auto" src='`+resp.volumen_caratula+`' alt='Novela Ligera `+resp.novela_nombre+` Volumen `+resp.volumen+`'>
                                    </div>
                                    <div class="vidvol">
                                    <div class="player" id="player">
                                        
                                    </div>
                                    <div class="covert" id="covert">
                                        <div class="espacio" id="espacio"><div id="vision" class="vision"></div><div id="partes" class="partes"><h5>PARTES</h5></div>`;
                                        star=comienzo(index);
                                        var sep = ",";
                                        var vids = resp.volumen_enlace_video.split(sep); 
                                        var index = index || 0; 
                                        for(var i=0;i<vids.length;i++)
                                        {
                                            if(i==0)
                                            {
                                                var col=`style="background-color:green"`;
                                            }
                                            else
                                            {
                                                var col=``;
                                            ...
                                            var b=i+1;
                                            template=template+` <div class="boton2" id="parte`+i+`" onclick="actuindex(`+i+`,'`+vids+`')" `+col+`>`+b+`</div>`;
                                        }
                                    template=template+`</div>
                                        <div class="play" id="playpause" onclick="playpause();">&nbsp;</div>
                                        <div class="timeri" id="timeri"></div>
                                    </div>
                                    <div class="bar" id="bar" style="width:100%; height: 15px; background-color: black;
                                    cursor: pointer; padding:0px; margin:0px; display:flex; flex-direction:row;" onclick="actualizarbarra()" onmousemove="mostrartiempo();" onmouseout="$('#progress-bar').empty();">
                                            <div class="progress-bar" id="progress-bar" style="height: 15px;
                                            background-color: red; width:0%; margin:0px; padding:0px; font-size: 14px; color:white;" onclick="actualizarbarra()"></div>
                                            <div id="cabezal" class="cabezal" style="pointer-events: none; user-select: none; height:15px; width:7.5px; background-color:#ccc;"></div>
                                        </div>
                                    </div>
                                </div>`;
    ...
    function pasar(callback)
    {
        ...
        var novela_nombre=$('#novelaNombre').val();
        
        $.ajax({
            type: "POST",
            url: "ajax/pasarvolumen.php",
            data: {volumen_id,novela_id,novela_nombre},
    ...
    function sumardescarga(volumen_id,novela_id)
    {
        $.ajax({
            url:'ajax/descargarvolumen.php',
            type:'POST',
            data:{volumen_id,novela_id},
            success:function(response){
            }
        })
    }
$(document).ready(function(){
    $('#newComent').click(function(){
        ...
        $.ajax({
            ...
            type:'POST',
            data:{usuario_id,novela_id,volumen_id,comentario},
            success:function(response){
    ...
    $('#newComent2').click(function(){
        ...
        var novela_id=$('#novelaId2').val();
        var volumen_id=$('#volumenId2').val();
        var comentario=$('#comentario2').val();
        $.ajax({
            url:'ajax/comentar.php',
            type:'POST',
            data:{usuario_id,novela_id,volumen_id,comentario,respuesta},
            success:function(response){
                //$('#pre-coment').html(response);
                $('#comentario2').val("");
                $('#comentarfixed').css('display','none');
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            var i=0;
            res.forEach(volu => {
                templa=`<form method='get' action='editarnovela.php' id='ese${i}'>
                            <input type='hidden' name='volumen_id'  value='${volu.volumen_id}'>
                            <input type='hidden' name='novela_id'  value='${volu.novela_id}'>
                            <input type='hidden' name='novela_nombre'  value='${volu.novela_nombre}'>
    ...
    $.ajax({
        ...
        success: function (resp) {
            ...
            var i=0;
            ress.forEach(nov => {
                templat=`<form method='get' action='editarnovela.php' id='este${i}'>
    ...
    $.ajax({
        ...
        success: function (respu) {
            ...
            var i=0;
            ress.forEach(top => {
                tem=`<form method='get' action='editarnovela.php' id='oter${i}'>
                            <input type='hidden' name='novela_id' id='novela_id1${i}' value='${top.novela_id}'>
                            <input type='hidden' name='novela_nombre' id='novela_nombre1${i}' value='${top.novela_nombre}'>
